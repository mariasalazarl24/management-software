 ...
 */
router.get('/password-resets', adminAuthMiddleware, requireAdminAccess, async (req: Request, res: Response) => {
  try {
    ...
    const total = await prisma.passwordResetLog.count();
 ...
 */
router.get('/password-resets/:resetId', adminAuthMiddleware, requireAdminAccess, async (req: Request, res: Response) => {
  try {
    const log = await prisma.passwordResetLog.findUnique({
 ...
 */
router.get('/logs', adminAuthMiddleware, requireAdminAccess, async (req: Request, res: Response) => {
  try {
    const limit = Math.min(parseInt(req.query.limit as string) || 50, 100);
    const offset = parseInt(req.query.offset as string) || 0;
 ...
 */
router.get('/logs/:logId', adminAuthMiddleware, requireAdminAccess, async (req: Request, res: Response) => {
  try {
    try {
 ...
 */
router.get('/admin-actions', adminAuthMiddleware, requireAdminAccess, async (req: Request, res: Response) => {
  try {
    const limit = Math.min(parseInt(req.query.limit as string) || 50, 100);
    const offset = parseInt(req.query.offset as string) || 0;
