 ...
 */
router.get('/summary', adminAuth_1.adminAuthMiddleware, adminAuth_1.requireAdminAccess, async (req, res) => {
    try {
        // Get client counts by status
        const [totalClients, activeClients, suspendedClients, cancelledClients] = await Promise.all([
            db_1.prisma.b2BClient.count(),
            db_1.prisma.b2BClient.count({ where: { status: 'ACTIVE' } }),
            db_1.prisma.b2BClient.count({ where: { status: 'SUSPENDED' } }),
            db_1.prisma.b2BClient.count({ where: { status: 'CANCELLED' } }),
        ]);
        // Get total users and buildings
        const [totalUsersResult, totalBuildingsResult] = await Promise.all([
            db_1.prisma.user.count(),
            db_1.prisma.building.count(),
        ]);
        // Get contracts expiring in 30 days
        const thirtyDaysFromNow = new Date();
        thirtyDaysFromNow.setDate(thirtyDaysFromNow.getDate() + 30);
        const contractsExpiringResult = await db_1.prisma.b2BClient.count({
            where: {
                status: 'ACTIVE',
                contractEndDate: {
 ...
 */
router.get('/clients-summary', adminAuth_1.adminAuthMiddleware, adminAuth_1.requireAdminAccess, async (req, res) => {
    try {
        const limit = Math.min(parseInt(req.query.limit) || 50, 100);
        const offset = parseInt(req.query.offset) || 0;
        const clients = await db_1.prisma.b2BClient.findMany({
            take: limit,
            skip: offset,
            select: {
                id: true,
        ...
        const total = await db_1.prisma.b2BClient.count();
        res.status(200).json({
            success: true,
 ...
 */
router.get('/revenue-summary', adminAuth_1.adminAuthMiddleware, adminAuth_1.requireAdminAccess, async (req, res) => {
    try {
        const now = new Date();
        const thirtyDaysFromNow = new Date();
        thirtyDaysFromNow.setDate(thirtyDaysFromNow.getDate() + 30);
        ...
        const [totalActive, expiringIn30Days, expired] = await Promise.all([
            ...
            db_1.prisma.b2BClient.findMany({
                where: {
                    status: 'ACTIVE',
                    contractEndDate: {
                        lte: thirtyDaysFromNow,
 ...
 */
router.get('/alerts', adminAuth_1.adminAuthMiddleware, adminAuth_1.requireAdminAccess, async (req, res) => {
    try {
        const alerts = [];
        const now = new Date();
        const thirtyDaysFromNow = new Date();
